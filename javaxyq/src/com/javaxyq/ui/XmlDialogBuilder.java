...
public class XmlDialogBuilder implements DialogBuilder{
	...
	public Panel processDialog(Node dlgEl) {
		...
		try {
			...
			try {
				int x = (GameMain.getGameCanvas().getWidth() - width)/2;
				int y = (GameMain.getGameCanvas().getHeight() - height)/2;
				dialog.setLocation( x, y);
			}catch(Exception ex) {}
		}
		...
		//加载面板的脚本
		PanelListener listener = GroovyScript.loadUIScript((String) dlgEl.get("@id"));
		if(listener!=null) {
...
//			}
	...
	public Label processText(Panel dialog,Node el) {
		...
		if (color!=null) {
			label.setForeground(UIHelper.getColor(color));
		}
		if (align != null) {
		...
		String tooltip = (String) el.get("@tooltip");
		if(tooltip != null)label.setTooltipTpl(tooltip);
		dialog.add(label);
		return label;
	}
...
//			editor.addActionListener(action);
		...
		if(foreground != null) {
			editor.setForeground(UIHelper.getColor(foreground));
		}
