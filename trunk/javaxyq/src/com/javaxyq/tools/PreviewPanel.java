...
import com.jidesoft.list.AbstractListFilter;
...
public class PreviewPanel extends JPanel {
	...
	public PreviewPanel(FileSystem filesystem) {
		...
		try {
			// FIXME load desc
			String descFile = "resources.desc";
			if (filesystem instanceof WdfFile) {
				WdfFile wdf = (WdfFile) filesystem;
				descFile = "resources/desc/" + new File(wdf.getName()).getName() + ".ini";
			}
			this.fileSystem.load(descFile);
		} catch (Exception ex) {
			System.err.printf("load wdf[%s] description failure.\n", filesystem);
			ex.printStackTrace();
		}
		// Collection<WdfFileNode> nodes = file.fileNodes();
		// this.fileNodes = new WdfFileNode[nodes.size()];
		// nodes.toArray(this.fileNodes);
			...
			protected Filter createFilter() {
				AbstractListFilter filter = new AbstractListFilter() {
					public boolean isValueFiltered(Object value) {
						...
						// FIXME fileter
						// WdfFileNode node = fileSystem.findNode(nodeId);
		...
		filterField.setHintText("Type here to filter fonts");
		toolbar.add(filterField);

		imagePreviewList = new ImagePreviewList(filterField.getDisplayListModel());
	...
	public void setPath(FileObject fileObject) {
		if (this.path == null || !this.path.equals(fileObject)) {
			this.path = fileObject;
			this.fileNodes = fileObject.listFiles();
	...
	private class PreviewImageIcon extends DefaultPreviewImageIcon {
		...
		public void setUserObject(FileObject userObject) {
			this.userObject = userObject;
			this.setTitle(userObject.getName());
			this.setDescription(userObject.getPath());
		}
